...
public interface ILikeService extends IService<FavoriteInfo> {
    ...
    List<ProductOutlineVo> showAllLike(String phone);      // 顯示所有收藏商品
    List<ProductOutlineVo> showLikeByOrder(String phone, Integer type);      // 按序顯示所有商品
    // (價格排序:高到低/低到高；加入時間:進到遠/遠到近)
    List<ProductOutlineVo> showLikeByCatalog(String phone, String catalog);  // 根據商品分類編號顯示收藏商品
